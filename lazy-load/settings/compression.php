...
function whippet_images_settings_compression() {
    if ( isset( $_POST['submit'] ) && isset( $_POST['wll_section'] ) && 'compression' === $_POST['wll_section'] ) {
        ...
        update_option( 'whippet_images_enable_compression', ! empty( $_POST['enable_compression'] ) ? 1 : 0 );
        update_option( 'whippet_images_quality', absint( $_POST['quality'] ?? 100 ) );
    }

    $enable_cdn = get_option('whippet_images_enable_cdn');
    $enable_compression = get_option('whippet_images_enable_compression');
    $quality = esc_attr(get_option('whippet_images_quality'));
    ...
    <form method="POST">
        ...
        <input type="hidden" name="wll_section" value="compression">
