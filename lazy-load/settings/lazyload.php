...
function whippet_images_settings_lazy_load() {

    if ( isset( $_POST['submit'] ) && isset( $_POST['wll_section'] ) && 'lazyload' === $_POST['wll_section'] ) {
        ...
        update_option( 'whippet_images_enable_lazyloading', ! empty( $_POST['enable_lazyloading'] ) ? 1 : 0 );
        update_option( 'whippet_images_lazymethod', sanitize_text_field( wp_unslash( $_POST['lazymethod'] ?? 'javascript' ) ) );

        $margin = isset( $_POST['margin'] ) ? absint( $_POST['margin'] ) : 0;
        update_option( 'whippet_images_margin', $margin );

        $raw_keywords = isset( $_POST['exclude_keywords'] ) ? sanitize_textarea_field( wp_unslash( $_POST['exclude_keywords'] ) ) : '';
        $keywords = trim( $raw_keywords ) ? array_map( 'trim', explode( "\n", str_replace( "\r", "", $raw_keywords ) ) ) : array();
        update_option( 'whippet_images_exclude_keywords', $keywords );
    }

    $enable_lazyloading = get_option('whippet_images_enable_lazyloading');
    $lazymethod = esc_attr(get_option('whippet_images_lazymethod'));
    $margin = esc_attr(get_option('whippet_images_margin'));
...
<form method="POST">
    ...
    <input type="hidden" name="wll_section" value="lazyload">
